...
export default function DocumentPreviewPage() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto pb-10 px-4 md:px-0 mt-4 md:mt-0 print:p-0 print:m-0">
      {/* Top Action Bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-2 mb-2 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[17px] font-bold text-gray-900">Document Preview</h1>
        </div>

        <div className="flex items-center gap-3 w-full md:w-auto">
          <Button 
            variant="outline" 
            className="rounded-[10px] flex-1 md:flex-auto font-medium px-4 h-10 border-gray-200 hover:bg-gray-50 bg-white text-gray-700 shadow-sm transition-colors"
          ...
          >
            {isDownloading ? (
               ...
               <Download className="w-4 h-4 mr-2.5 text-gray-600" strokeWidth={2} />
            )}
            Download PDF
          </Button>
          <Button 
            className="rounded-[10px] flex-1 md:flex-auto font-medium px-4 h-10 bg-[#003CBB] hover:bg-[#5585EA] text-white shadow-sm transition-colors"
            onClick={handlePrint}
          >
      ...
      <div className="w-full overflow-x-auto bg-transparent rounded-[24px] print:overflow-visible print:rounded-none">
        ...
        {/* The Paper Sheet */}
        <div id="printable-document" ref={documentRef} className="min-w-[900px] w-full max-w-5xl mx-auto bg-white border border-gray-100 shadow-[0_2px_15px_rgba(0,0,0,0.04)] rounded-[24px] relative overflow-hidden flex flex-col p-8 md:p-14 min-h-[800px] print:shadow-none print:border-none print:p-0 print:m-0 print:min-w-0 print:overflow-visible">
          
          {/* Faint Watermark */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.6] z-0 print:opacity-[0.6]">
             <Image 
                src="/images/bu_logo2.png" 
                alt="Babcock University Logo" 
                width={500}
                height={500}
                className="h-[500px] w-[500px] object-contain opacity-[0.6] print:opacity-[0.6]" 
                priority
              />
          </div>
          ...
          <div className="relative w-full z-10 flex flex-col flex-1 pb-10">
             <div className="grid grid-cols-[50px_1.5fr_3.5fr_1fr_1fr_1fr_1.5fr] bg-[#F8F9FB] rounded-[12px] py-4 px-6 items-center border border-gray-50 print:bg-[#F3F4F6] print:border-none">
               <div className="text-[13px] font-semibold text-gray-500">SN</div>
               <div className="text-[13px] font-semibold text-gray-500">Course code</div>
               <div className="text-[13px] font-semibold text-gray-500">Course Title</div>
               <div className="text-[13px] font-semibold text-gray-500 text-center">Units</div>
               <div className="text-[13px] font-semibold text-gray-500 text-center">Score</div>
               <div className="text-[13px] font-semibold text-gray-500 text-center">Grade</div>
               <div className="text-[13px] font-semibold text-gray-500 text-center">Grade Point</div>
             </div>

             <div className="flex flex-col mt-4 gap-2">
                {courses.map((course, idx) => (
                  <div key={idx} className="grid grid-cols-[50px_1.5fr_3.5fr_1fr_1fr_1fr_1.5fr] py-3.5 px-6 items-center text-[14px] text-gray-600 rounded-[8px] hover:bg-gray-50/50 transition-colors">
                    <div>{course.sn}</div>
